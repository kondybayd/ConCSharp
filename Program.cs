...
namespace ConsoleApp2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            /*string pass = "My88d";
            bool checkPass = CheckPassword(pass);

            if (checkPass) Console.WriteLine("Pass is OK");
            ...
            static bool CheckPassword(string pass)
            {
                ...
                BuildMessage("duman", 5);

                static void BuildMessage(string name, int age)
                {
                    Console.WriteLine($"Name:{name}, Age: {age}");
                }

                static int MaxOfNums(int a, int b)
                {
                    if (a > b) return a;
